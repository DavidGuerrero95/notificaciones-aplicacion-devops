...
//  ****************************	NOTIFICACIONES	***********************************  //
    ...
    public void enviarCodigoEditarContrasenia(@PathVariable("username") String username, @RequestParam(value = "email") String email,
                                              @RequestParam(value = "codigo") Integer codigo) throws IOException {
        nServices.enviarCodigoEditarContrasenia(username, email, codigo);
    }
...
//  ****************************	SUSCRIPCIONES	***********************************  //
    ...
    public void enviarMensajeSuscripciones(@RequestParam("idProyecto") Integer idProyecto,
        ...
        String nombreP = cbFactory.create("notificaciones").run(() -> pClient.obtenerNombre(idProyecto),
        ...
        List<List<String>> totalNotificacions = new ArrayList<List<String>>();
        Mensajes correo = new Mensajes();
        String mensaje = "Suscrito correctamente al proyecto: " + nombreP + ".";
    ...
    public void enviarMensajeInscripciones(@RequestParam("idProyecto") Integer idProyecto,
                                           @RequestParam("nombre") String nombre, @RequestParam("username") String username) {
        Notificaciones noti = new Notificaciones();
        List<String> notificacion = new ArrayList<>();
        List<List<String>> totalNotificacions = new ArrayList<List<String>>();
        Calendar c = Calendar.getInstance();
        ...
        String mensaje = cbFactory.create("notificaciones").run(() -> pClient.obtenerMensajeParticipacion(idProyecto),
                e -> encontrarProyecto(idProyecto, e));

        if (cbFactory.create("notificaciones").run(() -> pClient.obtenerEstadoGamificacion(idProyecto),
                e -> encontrarGamificacionProyecto(idProyecto, e)))
...
//  ****************************	FUNCIONES TOLERANCIA A FALLOS	***********************************  //
    ...
    private String encontrarProyecto(Integer idProyecto, Throwable e) {
        log.info(e.getMessage());
        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Servicio de proyectos no esta disponible");
    }

    private Boolean encontrarGamificacionProyecto(Integer idProyecto, Throwable e) {
        log.info(e.getMessage());
        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Servicio de proyectos no esta disponible");
    }
}
